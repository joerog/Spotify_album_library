...
CLIENT_SIDE_URL = "http://192.168.1.142"
PORT = 8080
...
# ---------------- 8. LIBRARY ------------------------
GET_LIBRARY_ENDPOINT = "{}/{}".format(SPOTIFY_API_URL, 'me')

def get_saved_albums(auth_header):
    empty = False
    offset = 0
    items = []
    while not empty:
        url = "{}/{}?limit={}&offset={}".format(GET_LIBRARY_ENDPOINT, 'albums', 50, offset)
        offset = offset + 50
        resp = requests.get(url, headers=auth_header)
        if resp.json().get('items') == None or len(resp.json().get('items')) == 0:
            empty = True
        else:
            items = items + resp.json().get('items')
        
    return items

def get_top(auth_header):
    url = "{}/{}?limit={}".format(GET_LIBRARY_ENDPOINT, 'top/artists', 50)
    resp = requests.get(url, headers=auth_header)
